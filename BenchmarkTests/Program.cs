...
namespace BenchmarkTests
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			ElasticSearchClient.Instance.Index("benchmark_test", "default", Guid.NewGuid().ToString(), data);
			...
			{
				var response = ElasticSearchClient.Instance.Index("benchmark_test", "default", Guid.NewGuid().ToString(), data);
				if (!response.Success)
				{
					failure++;
				}
			...
			ElasticSearchClient.Instance.DeleteIndex("benchmark_test");
