...
{
	internal class PooledConnectionProvider : NormalConnectionProvider
	{
				...
				else if (_freeConnections.Count + _usedConnections.Count >= PoolSize)
				{
					if (!Monitor.Wait(_lock, TimeSpan.FromSeconds(30)))
						throw new ElasticSearchException(
							"No connection could be made, timed out trying to aquire a connection from the connection pool.");
		...
		private void CheckFreeConnectionsAlive()
		{
			using (TimedLock.Lock(_lock))
			{
				IConnection[] freeConnections = _freeConnections.ToArray();
				_freeConnections.Clear();

				foreach (IConnection free in freeConnections)
				{
					if (IsAlive(free))
